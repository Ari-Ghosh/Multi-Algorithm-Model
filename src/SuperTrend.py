...
def get_st_signal(data):
    ...
    data["st_signal"] = pd.DataFrame(st_signal).rename(columns = {0:'st_signal'}).set_index(data.index)
    data["position"] = pd.DataFrame(position).rename(columns = {0:'st_position'}).set_index(data.index)
